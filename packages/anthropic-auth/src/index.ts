/**
 * Standalone Anthropic OAuth auth plugin for snow-code
 * No external subpath imports - PKCE inlined for Bun binary compatibility
 */
import type { Plugin } from "@groeimetai/snow-code-plugin"

const CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"

// Inlined base64url encoding (no external deps)
...
// Inlined PKCE generation (no @openauthjs/openauth/pkce import)
...
async function generatePKCE(length = 64): Promise<{ verifier: string; challenge: string }> {
...
export async function AnthropicAuthPlugin({ client }: Parameters<Plugin>[0]) {
  return {
    auth: {
      provider: "anthropic",
      async loader(
